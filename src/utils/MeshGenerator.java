...
public class MeshGenerator {
    ...
    public static @NotNull OGLBuffers generateGrid(final @NotNull GL2GL3 gl, final int m, final int n, final @NotNull String shaderName) {

        Seq<Vec2D> vertices = Stream.range(0, m).flatMap(
                (final Integer r) -> Stream.range(0, n).map(
                        (final Integer c) -> new Vec2D(c / (n - 1.0), r / (m - 1.0))
                )
        );
        ...
        Seq<Integer> indices = Stream.range(0, m - 1).flatMap(
                (final Integer r) -> Stream.range(0, n - 1).flatMap(
                        (final Integer c) -> offsets.map(
                                (final Tuple2<Integer, Integer> offset) -> (r + offset._1) * n + c + offset._2
                        )
        ...
        return new OGLBuffers(gl, ToFloatArray.convert(vertices.toJavaList()), attributes, ToIntArray.convert(indices.toJavaList()));
    }
